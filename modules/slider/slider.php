...
include_once(_PS_MODULE_DIR_.'slider/Slide.php');
...
class Slider extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Image bis slider for your homepage');
		$this->description = $this->l('Adds a second image slider to your homepage.');
	...
	public function install()
	{
		...
		)
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('SLIDER_WIDTH', $this->default_width, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_PAUSE', $this->default_pause, false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_LOOP', $this->default_loop, false, $shop_group_id, $shop_id);
			}
			...
			if (count($shop_groups_list))
			{
				...
				{
					$res = Configuration::updateValue('SLIDER_WIDTH', $this->default_width, false, $shop_group_id);
					$res &= Configuration::updateValue('SLIDER_SPEED', $this->default_speed, false, $shop_group_id);
					$res &= Configuration::updateValue('SLIDER_PAUSE', $this->default_pause, false, $shop_group_id);
					$res &= Configuration::updateValue('SLIDER_LOOP', $this->default_loop, false, $shop_group_id);
				}
			}

			/* Sets up Global configuration */
			$res = Configuration::updateValue('SLIDER_WIDTH', $this->default_width);
			$res &= Configuration::updateValue('SLIDER_SPEED', $this->default_speed);
			$res &= Configuration::updateValue('SLIDER_PAUSE', $this->default_pause);
			$res &= Configuration::updateValue('SLIDER_LOOP', $this->default_loop);
	...
	protected function installSamples()
	{
		...
		for ($i = 1; $i <= 3; ++$i)
		{
			...
			foreach ($languages as $language)
			{
				...
				$slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v16_slider'
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName('SLIDER_WIDTH');
			$res &= Configuration::deleteByName('SLIDER_SPEED');
			$res &= Configuration::deleteByName('SLIDER_PAUSE');
			$res &= Configuration::deleteByName('SLIDER_LOOP');
	...
	protected function createTables()
	{
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'slider_slides` (
			  `id_slider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_slider_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'slider_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'slider`, `'._DB_PREFIX_.'slider_slides`, `'._DB_PREFIX_.'slider_slides_lang`;
		');
	}
	...
	protected function _postValidation()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			if (!Validate::isInt(Tools::getValue('SLIDER_SPEED')) || !Validate::isInt(Tools::getValue('SLIDER_PAUSE')) ||
				!Validate::isInt(Tools::getValue('SLIDER_WIDTH'))
			)
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('SLIDER_WIDTH', (int)Tools::getValue('SLIDER_WIDTH'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_SPEED', (int)Tools::getValue('SLIDER_SPEED'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_PAUSE', (int)Tools::getValue('SLIDER_PAUSE'), false, $shop_group_id, $shop_id);
				$res &= Configuration::updateValue('SLIDER_LOOP', (int)Tools::getValue('SLIDER_LOOP'), false, $shop_group_id, $shop_id);
			}
			...
			switch ($shop_context)
			{
				case Shop::CONTEXT_ALL:
					$res = Configuration::updateValue('SLIDER_WIDTH', (int)Tools::getValue('SLIDER_WIDTH'));
					$res &= Configuration::updateValue('SLIDER_SPEED', (int)Tools::getValue('SLIDER_SPEED'));
					$res &= Configuration::updateValue('SLIDER_PAUSE', (int)Tools::getValue('SLIDER_PAUSE'));
					$res &= Configuration::updateValue('SLIDER_LOOP', (int)Tools::getValue('SLIDER_LOOP'));
					if (count($shop_groups_list))
					{
						foreach ($shop_groups_list as $shop_group_id)
						{
							$res = Configuration::updateValue('SLIDER_WIDTH', (int)Tools::getValue('SLIDER_WIDTH'), false, $shop_group_id);
							$res &= Configuration::updateValue('SLIDER_SPEED', (int)Tools::getValue('SLIDER_SPEED'), false, $shop_group_id);
							$res &= Configuration::updateValue('SLIDER_PAUSE', (int)Tools::getValue('SLIDER_PAUSE'), false, $shop_group_id);
							$res &= Configuration::updateValue('SLIDER_LOOP', (int)Tools::getValue('SLIDER_LOOP'), false, $shop_group_id);
						}
					}
					break;
	...
	protected function _prepareHook()
	{
		if (!$this->isCached('slider.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign(array('slider_slides' => $slides));
		}
	...
	public function hookdisplayHeader($params)
	{
		if (!isset($this->context->controller->php_self) || $this->context->controller->php_self != 'index')
			return;
		$this->context->controller->addCSS($this->_path.'slider.css');
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'slider (id_slider_slides, id_shop)
			SELECT id_slider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'slider
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'slider_slides` hss, `'._DB_PREFIX_.'slider` hs
			WHERE hss.`id_slider_slides` = hs.`id_slider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_slider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
			hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
			FROM '._DB_PREFIX_.'slider hs
			LEFT JOIN '._DB_PREFIX_.'slider_slides hss ON (hs.id_slider_slides = hss.id_slider_slides)
			LEFT JOIN '._DB_PREFIX_.'slider_slides_lang hssl ON (hss.id_slider_slides = hssl.id_slider_slides)
	...
	public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
	{
		...
		$results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'slider_slides hss ON (hs.id_slider_slides = hss.id_slider_slides)
			LEFT JOIN '._DB_PREFIX_.'slider_slides_lang hssl ON (hss.id_slider_slides = hssl.id_slider_slides)
			WHERE hs.`id_slider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
			($active ? ' AND hss.`active` = 1' : ' ')
		);
	...
	{
		$req = 'SELECT hs.`id_slider_slides` as id_slide
				FROM `'._DB_PREFIX_.'slider` hs
				WHERE hs.`id_slider_slides` = '.(int)$id_slide;
	...
	public function getConfigFieldsValues()
	{
		...
		return array(
			'SLIDER_WIDTH' => Tools::getValue('SLIDER_WIDTH', Configuration::get('SLIDER_WIDTH', null, $id_shop_group, $id_shop)),
			'SLIDER_SPEED' => Tools::getValue('SLIDER_SPEED', Configuration::get('SLIDER_SPEED', null, $id_shop_group, $id_shop)),
			'SLIDER_PAUSE' => Tools::getValue('SLIDER_PAUSE', Configuration::get('SLIDER_PAUSE', null, $id_shop_group, $id_shop)),
			'SLIDER_LOOP' => Tools::getValue('SLIDER_LOOP', Configuration::get('SLIDER_LOOP', null, $id_shop_group, $id_shop)),
		);
	}
